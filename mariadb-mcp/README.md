...
MariaDB 数据库的 MCP 服务器，统一权限控制。MariaDB 兼容 MySQL 协议。
...
| 变量 | 说明 |
|------|------|
| `MARIADB_URL` | 连接字符串，如 `mysql://user:pass@host:3306/db` |
| `MARIADB_HOST` | 主机地址，默认 `localhost` |
| `MARIADB_PORT` | 端口，默认 `3306` |
| `MARIADB_USER` | 用户名，默认 `root` |
| `MARIADB_PASSWORD` | 密码 |
| `MARIADB_DATABASE` | 数据库名，默认 `test` |
