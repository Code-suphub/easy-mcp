...
MCP server for the TiDB distributed database with unified permission control. TiDB speaks the MySQL protocol.
...
|------|------|
| `TIDB_URL` | Connection string, e.g. `mysql://user:pass@host:4000/db` |
| `TIDB_HOST` | Host, default `localhost` |
| `TIDB_PORT` | Port, default `4000` |
| `TIDB_USER` | Username, default `root` |
| `TIDB_PASSWORD` | Password |
| `TIDB_DATABASE` | Database name, default `test` |
| `TIDB_SSL` | Set to `true` to enable SSL (for TiDB Cloud) |
