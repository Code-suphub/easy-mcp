...
TiDB 分布式数据库的 MCP 服务器，统一权限控制。TiDB 兼容 MySQL 协议。
...
| 变量 | 说明 |
|------|------|
| `TIDB_URL` | 连接字符串，如 `mysql://user:pass@host:4000/db` |
| `TIDB_HOST` | 主机地址，默认 `localhost` |
| `TIDB_PORT` | 端口，默认 `4000` |
| `TIDB_USER` | 用户名，默认 `root` |
| `TIDB_PASSWORD` | 密码 |
| `TIDB_DATABASE` | 数据库名，默认 `test` |
| `TIDB_SSL` | 设为 `true` 启用 SSL（适配 TiDB Cloud） |
