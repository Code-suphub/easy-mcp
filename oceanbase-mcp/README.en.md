...
MCP server for the OceanBase distributed database with unified permission control. OceanBase speaks the MySQL protocol.
...
```json
{
  "mcpServers": {
    "oceanbase": {
      ...
      "args": [
        "-y",
        "@easy-mcps/oceanbase-mcp-server"
      ]
    }
  }
}
...
|------|------|
| `OCEANBASE_URL` | Connection string, e.g. `mysql://user:pass@host:2881/db` |
| `OCEANBASE_HOST` | Host, default `localhost` |
| `OCEANBASE_PORT` | Port, default `2881` |
| `OCEANBASE_USER` | Username, default `root` |
| `OCEANBASE_PASSWORD` | Password |
| `OCEANBASE_DATABASE` | Database name, default `test` |
