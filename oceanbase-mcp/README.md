...
OceanBase 分布式数据库的 MCP 服务器，统一权限控制。OceanBase 兼容 MySQL 协议。
...
| 变量 | 说明 |
|------|------|
| `OCEANBASE_URL` | 连接字符串，如 `mysql://user:pass@host:2881/db` |
| `OCEANBASE_HOST` | 主机地址，默认 `localhost` |
| `OCEANBASE_PORT` | 端口，默认 `2881` |
| `OCEANBASE_USER` | 用户名，默认 `root` |
| `OCEANBASE_PASSWORD` | 密码 |
| `OCEANBASE_DATABASE` | 数据库名，默认 `test` |
